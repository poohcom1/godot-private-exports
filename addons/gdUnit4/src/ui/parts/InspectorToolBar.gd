...
func init_buttons() -> void:
	...
	var editiorTheme := EditorInterface.get_base_control().theme
	_button_run_overall.icon = overall_icon_image
	_button_run_overall.visible = GdUnitSettings.is_inspector_toolbar_button_show()
	_button_run.icon = get_theme_icon("Play", "EditorIcons")
	_button_run_debug.icon = debug_icon_image
	_button_stop.icon = get_theme_icon("Stop", "EditorIcons")
	_tool_button.icon = get_theme_icon("Tools", "EditorIcons")
	_button_wiki.icon = get_theme_icon("HelpSearch", "EditorIcons")
